...
def summarize_directory(directory: str):
    ...
    for file in files:
        with open(file, "r") as f:
            collection = jmaji.collect(jmaji.get_parse_tree(f.read()), jmaji.GismuCollector)
    ...
    table.add_row("total", str(len(total_glossed_gismus)), str(len(total_unglossed_gismus)), str(len(total_uncaught_gismus)))
    return table
